    ...
    def retrieve_pdb_file(self, pdb_code, obsolete=False, pdir=None, file_format=None, overwrite=False):
        ...
        if file_format not in archive.keys():
            raise (
                ...
                "typo. Please, use one of the following: mmCif, pdb, xml, mmtf, bundle" % file_format
            )
    ...
    def update_pdb(self, file_format=None):
        ...
        for pdb_code in obsolete:
            if self.flat_tree:
                ...
                old_file = os.path.join(self.local_pdb, pdb_code[1:3], f"pdb{pdb_code}.ent")
...
if __name__ == "__main__":
    genename2emsebl_pdb_path = Path("../data/mm_data/genename2emsebl_pdb.json")
    ...
    client = PDBList(pdb="../data/ppi_data/pdb")
