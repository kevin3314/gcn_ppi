...
if [ $numerical = true ]; then
    ...
    do
        ...
        do
            ...
            if [ $text = true ]; then
                if [ $graph = true ]; then
                    CMD_WITH_LOW_TENSORFUSION="${RUN_CMD} model.with_lowrank_tensorfusion_network=true datamodule.batch_size=8 +trainer.accumulate_grad_batches=2"
                    echo "run: $CMD_WITH_LOW_TENSORFUSION"
                    eval $CMD_WITH_LOW_TENSORFUSION

                    CMD_WITH_TENSORFUSION="${RUN_CMD} model.with_tensorfusion_network=true datamodule.batch_size=8 +trainer.accumulate_grad_batches=2"
